...
function buildPage(arr, type = TV) {
	...
	if(array.length === 0) {
		...
		for(i = 0; i < array.length; ++i) {
			...
			dataTable += '<button type="button" onclick="infoKitsu(\'' + array[i].name +'\', \'' + array[i].kitsu_id +'\')" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="caret"></span></button>';
			...
			dataTable += '<li role="presentation" id="info_' + noSpace(array[i].name) + '" class="info" ></li>';
			dataTable += '</ul></div></td>';
